...
SCENES = [
    ("intro",
     "Safe is the multisig most DAOs keep their money in. Several owners, and a payout "
     "needs a set number of them to sign. GhostLedger is a module you add to one. "
     "It compares every proposed payout against what that treasury normally spends, and "
     "makes an unusual payout need more signatures than a routine one. It does that "
     "comparison inside a sealed processor that hands back a verdict and not the number. "
     "So the treasury learns that a payout is large without anyone, including us, "
     "learning how large."),
    ...
    ("trust",
     "The amount never leaves its encrypted handle. The contract that scores it holds a "
     "reference, never a value, and it cannot read what it is judging. "
     "The verdict comes back signed, and the contract verifies that signature before it "
     "will accept a band at all. Hand it a forged one, and the transaction reverts."),

    ("safe",
     "Safe's own contract is untouched. GhostLedger installs through enableModule, which is "
     "Safe's own extension point, and it can only ever raise the number of signatures a "
     "payout needs. Never lower it."),
